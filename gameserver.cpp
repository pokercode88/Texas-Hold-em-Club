...
namespace game
{
    namespace gameserver
    {
        ...
        int GameServer::onGameSoMessage(const long lPlayerID, const vector<char> &vecMsgData)
        {
            ...
            //校验版本
            if (somsg.nver() != XGameBPProto::iProtoVersion)
            {
                LOG_WARN("client proto version : \"" << somsg.nver() << "\" server proto version : \"" << XGameBPProto::iProtoVersion << "\";");
            }

            //校验命令字
            if (somsg.ncmd() > XGameBPProto::E_BP_MSG_ID_END || somsg.ncmd() < XGameBPProto::E_BP_MSGID_BEGIN)
            {
        ...
        int GameServer::doSendGameData(const long &pid, int eMSG, const vector<char> &vecData)
        {
            ...
            sTSoMsg.set_nver(XGameBPProto::iProtoVersion);
